...
Blue Onyx offers flexible configuration options:
...
### Hybrid Approach
Combine configuration files with command-line overrides (CLI arguments take precedence):

```bash
blue_onyx --config production.json --port 9090 --log_level Debug
```
...
You can programmatically save the current configuration:
...
let config = Cli::from_config_and_args()?;

// Save to a new file
config.save_config(&PathBuf::from("my_config.json"))?;
```
...
- Ensure `blue-onyx-config-service.json` is in the same directory as the executable
