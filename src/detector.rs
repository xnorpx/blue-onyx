...
fn initialize_onnx(onnx_config: &OnnxConfig) -> InitializeOnnxResult {
    ...
    let (num_intra_threads, num_inter_threads) = if onnx_config.force_cpu {
        let num_intra_threads = onnx_config.intra_threads.min(num_cpus::get_physical() - 1).min(16);
        let num_inter_threads = onnx_config.inter_threads.min(num_cpus::get_physical() - 1).min(16);
        info!(
